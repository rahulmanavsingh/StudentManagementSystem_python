...
def Save():
    ...
    if N=="" or C1=="Select Class" or  D2=="" or Re1=="" or S1=="" or fathername=="" or mothername=="" or F1=="" or M1=="":
        ...
        regirstarion_no() #it will recheck registration no and issue new no.


#########################-----------SEARCH---------------------#########################
def search():
    
    text= Search.get() #taking input from enty box

    Clear() # to clear  all the data  available in the entry box and other
    saveButton.config(state='disable') #after clicking  on serach, save button will be disabled so that noone can click on it
    ...
    #reg_no_position showing like A2,A3,A4,A5,......An
    ...
    img = (Image.open("Student Images/"+str(x1)+".jpg")) # we done this to take image name as registraion no.
    resized_image=img.resize((190,190))
    photo2 = ImageTk.PhotoImage(resized_image)
    lbl.config(image=photo2)
    lbl.image=photo2
...
def Update():
    ...
    # sheet.cell(column=1,row=int(reg_number),value=R1) ------------>>>>>no one can update the registration no<<<-------
    ...
    Clear() #after update it will clear all the feilds
...
#top frames
Label(root,text="Email: rahulroy8843@gmail.com", width=10,height=3,bg="#f0687c",anchor='e').pack(side=TOP,fill=X)
...
upadate_button=Button(root,image=imageicon4,bg="#c36464",command=Update)
upadate_button.place(x=110,y=64)
...
regirstarion_no() #called it here 
...
M_Name=StringVar()
f_entry=Entry(obj2,textvariable=M_Name,width=20,font="arial 10")
f_entry.place(x=630,y=50)
